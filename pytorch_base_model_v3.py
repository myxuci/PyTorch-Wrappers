...
class PyTorchBaseModel(object):
    ...
    def fit(self, train_loader, test_loader, epoch, loss_function=None, optimizer=None, learning_rate=0.001, \
            learning_rate_decay_rate=0.8, lr_decay_tolerance=3, clip=-1, use_cuda=True, verbose=1, random_seed=7, **kwargs):
        '''
        :param train_loader:
        :param epoch:
        :param use_cuda:
        ...
        try:
            ...
            if hasattr(self.model, "trainable_params"):
                ...
                logging.info("Will only train partial parameters under tranable_params method.")
            else:
            ...
            for ep in range(1, epoch+1):
                # if ep == 1:
                #     # Define optimizer: (May be integrated into set parameter function.)
                #     _optimizer = self.set_optimizer(optimizer=optimizer)
                #     _optimizer = _optimizer(trainable_params, lr=adjustable_learning_rate)
                # if ep == 2:
                #     # Define optimizer: (May be integrated into set parameter function.)
                #     adjustable_learning_rate *= 0.1
                #     for param_group_ in _optimizer.param_groups:
                #         param_group_["lr"] = adjustable_learning_rate
                #     logging.info("Epoch {}, updated learning rate to {}".format(ep, adjustable_learning_rate))
                # elif _tolerance >= lr_decay_tolerance:
                #     self.model = _best_model # Restore the best model.
                #     _tolerance = 0 # Reset tolerance
                #     adjustable_learning_rate *= learning_rate_decay_rate
                #     for param_group_ in _optimizer.param_groups:
                #         param_group_["lr"] = adjustable_learning_rate
                #     _msg = "Loss did not improve for {} epochs. Adjusted learning rate to {}".format(lr_decay_tolerance, \
                ...
                for _train_loader_idx, _train_loader in enumerate(train_loader_list):
                    ...
                    _separator = '='*35 + '\tSession of data loader {}\t'.format(_train_loader_idx+1) + '='*35
                    print(_separator)
                    ...
                    for batch_idx, (data, target) in enumerate(_train_loader):
                        ...
                        if batch_idx % verbose == 0:
                            print('Epoch: {} Data Loader {} [{}/{} ({:.0f}%)]\t Avg Train Loss per Batch: {:.6f}'.format(\
                                                                            ep, _train_loader_idx+1, \
                                                                            batch_idx * len(data), \
                                                                            len(_train_loader.dataset), \
                                                                            100. * batch_idx / len(_train_loader), \
                ...
                avg_epoch_loss = _loss.item() # / batch_idx # Debug.

                # Logging and output epoch loss:
                epoch_loss_info = "Epoch {}, Average Loss: {:.6f}".format(ep, avg_epoch_loss)
                logging.info(epoch_loss_info)
                print(epoch_loss_info)

                # Calculate score from evaluation data set:
                test_log_loss = self.eval(self.model, test_loader)
                # logging.info("Epoch {}, Test Loss: {:.6f}, Test Accuracy: {:.2f}".format(ep, test_loss, test_accuracy))

                # Show error per epoch:
                # if ep % verbose == 0:
                    # print("Training Epoch: {}\tAvg Loss: {:.6f}".format(ep, avg_epoch_loss))

                if test_log_loss <= _best_loss:
                    _best_loss = test_log_loss
                    _best_epoch = ep
                    _best_model = self.model
                    _tolerance = 1
                else:
                    if _tolerance < lr_decay_tolerance:
                        _tolerance += 1
    ...
    def eval(self, model, test_loaders):
        ...
        try:
            ...
            for test_loader_idx, test_loader in enumerate(test_loader_list):
                ...
                for batch_idx, (data, target) in enumerate(test_loader):
                    ...
                    proba += proba_.cpu().detach().numpy().tolist()
