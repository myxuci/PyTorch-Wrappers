...
class PyTorchBaseModel(object):
    ...
    def fit(self, train_loader, test_loader, epoch, loss_function=None, optimizer=None, learning_rate=0.001, \
            learning_rate_decay_rate=0.8, lr_decay_tolerance=3, clip=-1, use_cuda=True, verbose=1, random_seed=7, \
        ...
        try:
            ...
            for ep in range(1, epoch+1):
                ...
                #     trainable_params = self.model.parameters()
                # for param_group_ in _optimizer.param_groups:
                #     param_group_["params"] = trainable_params
                #     logging.info("Epoch {}, updated the trainable parameters to all model parameters.")
                        ...
                        if ((batch_idx + 1) % verbose == 0):
                            print('Epoch: {} Data Loader {} [{}/{} ({:.0f}%)]\t Avg Train Loss per Batch: {:.6f}'.format( \
                                ...
                                len(_train_loader.dataset), \
                                    100 * (batch_idx + 1) / len(_train_loader), \
                                    _batch_train_loss/verbose))
