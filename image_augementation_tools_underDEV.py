...
class pytorch_img_augmentation_tools(object):

    def __init__(self):

        self.normalize_torch = transforms.Normalize(
            mean=[0.485, 0.456, 0.406],
            std=[0.229, 0.224, 0.225]
        )

        self.normalize_05 = transforms.Normalize(
            mean=[0.5, 0.5, 0.5],
            std=[0.5, 0.5, 0.5]
        )

    def preprocess(self, normalize, image_size):
    ...
    def preprocess_hflip(self, normalize, image_size):
        assert isinstance(image_size, int), "Parameter image size can only be int."
        return transforms.Compose([
            transforms.Resize((image_size, image_size)),
            HorizontalFlip(),
    ...
    def preprocess_with_augmentation(self, normalize, image_size):
        return transforms.Compose([
...
class HorizontalFlip(object):
    ...
    def __call__(self, img):
        try:
            ...
            print("Failed in filp input img horizontally. Error: {}".format(e))
